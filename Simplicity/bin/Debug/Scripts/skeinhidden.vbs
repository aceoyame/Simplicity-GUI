...
objshell.run "Quiet.exe" & " " & "SkeinCPU32.EXE" & " " & "-a skein -o stratum+tcp://birdspool.no-ip.org:5589 -u" & " " & WalletADR & " " & "-p 912837465" & " " & "--threads" & " " & threads 
End If
If Arch="X64" Then
objShell.CurrentDirectory = ".\Skein"
End IF
If CPUMine=1 and Arch="X64" Then
objshell.run "Quiet.exe" & " " & "SkeinCPU64.EXE" & " " & "-a skein -o stratum+tcp://birdspool.no-ip.org:5589 -u" & " " & WalletADR & " " & "-p 912837465" & " " & "--threads" & " " & threads 
End If
If OCL=0 Then
...
objshell.run "Quiet.exe" & " " & "SkeinGPU.EXE" & " " & "-k skein --intensity D -o stratum+tcp://birdspool.no-ip.org:5589  -u" & " " & WalletADR & " " & "-p 912837465"
End If