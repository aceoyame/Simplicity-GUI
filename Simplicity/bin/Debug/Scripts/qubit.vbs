...
objshell.run "QubitCPU32.EXE" & " " & "-a qubit -o stratum+tcp://birdspool.no-ip.org:5567 -u" & " " & WalletADR & " " & "-p 912837465" & " " & "--threads" & " " & threads
End If
If Arch="X64" Then
objShell.CurrentDirectory = ".\Qubit\"
End If
If CPUmine=1 and Arch="X64" Then
objshell.run "QubitCPU64.EXE" & " " & "-a qubit -o stratum+tcp://birdspool.no-ip.org:5567 -u" & " " & WalletADR & " " & "-p 912837465" & " " & "--threads" & " " & threads
End If
If OCL=1 and Cuda = 0 Then
objShell.CurrentDirectory = ".\QubitGPU"
objshell.run "QubitGPU.EXE" & " " & "--kernel qubitcoin --intensity D -o stratum+tcp://birdspool.no-ip.org:5567  -u" & " " & WalletADR & " " & "-p 912837465"
End If
If OCL=1 and Cuda = 1 Then 
objShell.CurrentDirectory = ".\QubitGPU"
 objShell.run "NVGPU.EXE" & " " & "--algo=qubit -o stratum+tcp://birdspool.no-ip.org:5567 -u" & " " & WalletADR & " " & "-p 912837465"
End If
If AMD=1 And Arch="X64"  And CpuMine=1 Then
objShell.CurrentDirectory = ".\Qubit\"
objshell.run "AMDCPU64.EXE" & " " & "-a qubit -o stratum+tcp://birdspool.no-ip.org:5567 -u" & " " & WalletADR & " " & "-p 912837465" & " " & "--threads" & " " & threads 
End IF
